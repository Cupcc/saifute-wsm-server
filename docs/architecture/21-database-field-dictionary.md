...
### 14.1 `system_management_snapshot` — 系统管理快照


| 字段名           | 数据类型        | 必填  | 默认值     | 唯一  | 说明                 |
| ------------- | ----------- | --- | ------- | --- | ------------------ |
| `id`          | INT         | 是   | 自增      | PK  | 主键                 |
| `snapshotKey` | VARCHAR(64) | 是   | —       | 是   | 快照键（如初始化状态标记），全局唯一 |
| `payload`     | JSON        | 是   | —       | —   | 快照数据（JSON 结构）      |
| `createdAt`   | DATETIME    | 是   | `now()` | —   | 创建时间               |
