...
- **成品入库**（生产完工入库）统一走 `inbound`（生产入库单），与验收单共表；产品「生产车间」在组织与权限上按**部门**划分，且业务侧按部门筛选时以**管理员配置的可见部门子集**为准（非默认平铺全公司部门），`workshop` 主档仍表示归属/核算维度，与 `workshop-material`（领退料报废）边界分离。
...
- 与 RD 小仓有关的项目库存动作默认来自 `RD_SUB`；项目本身不形成新的物理库存池
...
- 共享核心：`Material`、`Workshop`、`StockScope`、`InventoryBalance`、`InventoryLog`、`InventorySourceUsage`、`WorkflowAuditDocument`
