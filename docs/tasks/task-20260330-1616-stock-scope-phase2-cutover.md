...
- Related requirement: `docs/requirements/req-20260330-1616-stock-scope-phase2-cutover.md`
- Status: `planned`
- Review status: `not-reviewed`
- Lifecycle disposition: `active`
- Planner: `planner`
- Coder: `pending-user-confirmation`
...
- Requirement doc: `docs/requirements/req-20260330-1616-stock-scope-phase2-cutover.md`
- User intent summary:
...
- Open questions requiring user confirmation:
  - 选择哪一种 cutover 主策略：`schema expand + one-shot flip`、`schema expand + dual-write`，还是更激进的 `destructive one-shot`。
  - Phase 2 首波是否只覆盖库存核心与主单据家族，还是同时纳入 `rd-subwarehouse` 的 `handoff/stocktake/procurement` 持久化表。
  - `inventory_balance` / `inventory_log` / `inventory_source_usage` / `factory_number_reservation` 是走“原位 backfill + 切换”还是“按正式业务事实 replay / rebuild”。
  - 可接受的迁移窗口与停写方式是什么：短维护窗、只读窗，还是更长的冻结窗口。
  - rollback 是以“整库快照恢复”为主，还是要求提供细粒度 SQL/脚本回退路径。

## Requirement Sync

- Req-facing phase progress: `Phase 2` cutover brief 已写入本 task（策略分段、风险面、确认项齐备）；尚未进入 schema / migration / 代码实施。
- Req-facing current state: 运行时仍以 Phase 1 canonical `stockScope` 为真源；`inventory-core` 仍为唯一库存写入口；Prisma / DB 与大量单据行仍冻结在 `workshopId` 存储轴，与架构真源中的 `materialId + stockScopeId` 目标态不一致。
- Req-facing blockers: None（决策未冻结前，实施侧保持停手；是否将「待用户确认关键决策」记为阻塞由 parent 与用户对齐习惯）。
- Req-facing next step: 用户确认 `## User Decisions To Confirm`；可选开 `docs/workspace/stock-scope-phase2-cutover/` 做映射表与 rehearsal runbook；决策冻结后另开或升格为执行 slice（coder 可写范围须单独写明）。
...
- [ ] 首波 cutover 采用哪种主策略：`Option A`、`Option B` 还是 `Option C`。
- [ ] 首波纳入范围是否只含 `inventory-core + reporting + inbound/customer/workshop-material/project`，还是同步纳入 `rd-subwarehouse` 持久化表。
- [ ] `inventory_balance` / `inventory_log` / `inventory_source_usage` / `factory_number_reservation` 采用“原位 backfill”还是“按正式业务事实 replay / rebuild”。
- [ ] 是否允许维护窗口内的库存停写 / 只读模式；可接受窗口长度上限是多少。
- [ ] rollback 是否以“整库快照恢复 + 应用版本回退”为主，还是要求额外准备更细粒度的回退脚本。
- [ ] 是否需要 parent 立刻开 `docs/workspace/<workflow>/` 做更细的策略对比、例外映射和 rehearsal runbook。
...
- [ ] Step 1. 由用户确认主策略、首波纳入范围、backfill vs replay 口径、迁移窗口与 rollback 基线。
- [ ] Step 2. 基于确认结果补充更细的 schema/data/runtime matrix，包括哪些表必须同波切、哪些可后置 cleanup。
- [ ] Step 3. 产出 rehearsal 设计：映射表、对账 SQL、focused tests、e2e/stub 迁移、cutover gate 与 rollback gate。
- [ ] Step 4. 只有在上面三步完成且用户明确放行后，才新开或更新执行 task，给 coder 分配可写范围。

## Coder Handoff

- Execution brief:
  - 本 task 当前不是“开始实施”的授权文档，而是 `plan-only` cutover brief。
  - 在用户未确认 `## User Decisions To Confirm` 之前，coder 不应开始改 Prisma、migration、runtime 或 tests。
  - 如果 parent 决定进入实施，优先按“schema/data/runtime 分层但共享 contract 单 owner”的方式再开执行 slice，而不是直接拿本 brief 当施工单。
- Required source docs or files:
  ...
  - `prisma/schema.prisma`
- Owned paths:
  - `None until user confirmation`
  - expected future execution surfaces: `prisma/**`, `src/modules/inventory-core/**`, `src/modules/reporting/**`, `src/modules/inbound/**`, `src/modules/customer/**`, `src/modules/workshop-material/**`, `src/modules/project/**`, `src/modules/rd-subwarehouse/**`, `src/modules/session/**`, `src/modules/rbac/**`, `test/**`
...
- Validation command for this scope:
  - current planner scope: docs-only re-read of this task plus linked requirement / architecture / Phase 1 task
  - future execution minimum gate after approval: `pnpm prisma:generate`, `pnpm typecheck`, focused module tests, relevant e2e / stub updates, `pnpm test`, and DB-clone rehearsal with reconciliation reports
...
- Validation results:
  ...
  - Confirmed this planner slice only edits `docs/tasks/**`.
  - No runtime command or test gate was run because this slice is planning-only and does not change application code.
- Findings:
  - none yet; the remaining blockers are decision blockers, not implementation findings.
- Follow-up action:
  - wait for user confirmation on strategy, scope, migration window, and rollback baseline before opening execution work.

## Final Status

- Outcome: `active - planning brief created, awaiting user confirmation`
- Requirement alignment: 本 task 与 `docs/requirements/req-20260330-1616-stock-scope-phase2-cutover.md` 对齐，明确把 Phase 2 作为独立 schema/data cutover 规划切片，延续 Phase 1 已完成的 canonical runtime contract，而不是重新混入 runtime 语义收敛。
- Residual risks or testing gaps:
  - 尚未冻结 cutover 策略、首波范围、backfill/replay 口径、迁移窗口与 rollback 基线。
  - 尚未进入实现，因此也尚未跑 Prisma / typecheck / tests / rehearsal；这些都属于后续执行 slice 的 gate，而不是本轮 planner slice 的缺失。
- Directory disposition after completion: keep `active` until user confirms decisions and the follow-up execution / review slices are completed, then sync `docs/tasks/TASK_CENTER.md`
- Next action: 由用户确认 `## User Decisions To Confirm`；若继续细化方案，建议 parent 开 `docs/workspace/stock-scope-phase2-cutover/` 并保持 `plan-only`
