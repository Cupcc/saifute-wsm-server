# Project Management Phase 1-2 Delivery

## Metadata

- Scope:
  - 完成 `docs/requirements/domain/project-management.md` 的 `F1/F2/F3/F4`，将当前仅覆盖“项目消耗单”的 `project` 模块收敛为符合需求真源的项目主档、轻量 `BOM`、缺料/补货视图、项目库存动作归集与项目净耗用/成本台账。
  - 以当前仓库现实为约束：已有 `src/modules/project/**` 会在 `create/update/void` 时直接通过 `inventory-core` 记 `PROJECT_CONSUMPTION_OUT`，前端 `web/src/views/rd/project-consumption/index.vue` 也是“研发项目领用”页面；本 task 需要把它提升到 Phase 1/2 所需产品形态，而不是继续把“项目管理”实现成单一消耗单页面。
  - 明确排除 `F5` customer 销售引用项目成品成本快照，以及任何完整项目进度/状态管理。
- Related requirement: `docs/requirements/domain/project-management.md (F1,F2,F3,F4)`
- Status: `completed`
...
- Related acceptance spec: `docs/acceptance-tests/specs/project-management.md`
- Related acceptance run: `docs/acceptance-tests/runs/run-20260409-0126-project-management-phase1-phase2.md`
- Related files:
  ...
  - `web/src/views/rd/project-consumption/index.vue`
...
- Domain capability:
  - `docs/requirements/domain/project-management.md (F1,F2,F3,F4)`
  - 仅承接 `Phase 1` 与 `Phase 2`；`F5` 明确不在本 task 范围内。
- User intent summary:
  - 用户明确要求“`project-management.md` 实现 phase 1,phase2”。
  - 父级已限定本次 planning 只为 `project-management` 新建 durable active task doc，并要求以当前代码现实为准，不发明需求变化。
- Acceptance criteria carried into this task:
  ...
  - 前端必须提供项目管理入口与页面闭环；当前 `project-consumption` 页面若继续复用，需被升级为满足 `F1/F2/F4` 的项目管理界面，而不是保持“新增项目领用”单一页面。
- Requirement evidence expectations:
  ...
  - `docs/acceptance-tests/specs/project-management.md` 为完整测试报告载体；必要时补 `cases` / `run`。
...
- Current state:
  - `src/modules/project/**` 已存在并已接入 `allocation_target = RD_PROJECT`，但现状主要是“项目物料消耗单 + 直接库存过账”。
  - `web/src/views/rd/project-consumption/index.vue` 现状是 RD 侧“研发项目领用”页面，不是项目主档/BOM/台账页面。
  - 本次 planning 已补建 `project-management` active task；补建前 `TASK_CENTER` 中无对应项。`F1/F3` 在 requirement 中标为进行中，但实现形态与 domain 目标仍存在明显偏差。
- Acceptance state:
  - `not-assessed`
- Blockers:
  - `none for planning`
...
- Goal:
  - 在不引入项目状态系统、不提前推进 `F5` 的前提下，完成 `project-management` 的 `Phase 1` 与 `Phase 2`：项目主档、轻量 `BOM`、缺料/补货、项目库存动作归集、净耗用/成本台账与前端入口全部落到当前仓库的真实模块边界上。
- Acceptance criteria:
  ...
  - `[AC-3]` `F3` 落地：项目领料/退料/报废的库存事实统一通过 `inventory-core` 落账，并保留 `RD_PROJECT` 归集与来源追溯；实现不会与 `workshop-material` 现有 `pick/return/scrap` 家族形成两套并行且不一致的运行时语义。
  - `[AC-4]` `F4` 落地：系统可按项目查看 `计划成本、领料成本、退料回补、报废损耗、净耗用数量/金额`，并能追溯到相关库存/来源使用与 RD 协同结果。
  - `[AC-5]` 前端闭环：项目管理入口支持项目主档、`BOM`、缺料/补货信息与台账查询；若保留现有 `project-consumption` 页面，则其语义与交互必须完成 Phase 1/2 升级，不再只是“新增项目领用”。
...
- Frozen or shared paths:
  ...
  - `src/modules/customer/**`、`src/modules/reporting/**`、`docs/workspace/**`、与 `F5` 相关的任何表面均冻结。
- Task doc owner:
  - `parent orchestrator`
- Contracts that must not change silently:
  - `project-management` 不是项目状态管理系统，不新增 `立项/进行中/关闭` 等完整生命周期。
  - `RD_SUB` 仍是项目实际作业仓别；`MAIN` 继续承担验收与向 `RD_SUB` 交接，不直接作为项目实际使用仓别。
  - `inventory-core` 仍是库存唯一写入口。
  - `rd-subwarehouse` 继续拥有 RD 协同与采购/交接状态链；项目域只消费其结果做缺料、补货与成本归集。
  - `workshop-material` 已有 `pick/return/scrap` 语义是共享边界；Phase 2 不得复制出第二套长期并行单据家族。
  - `customer` `F5` 项目成品成本快照明确不在本 task 范围内。

## Implementation Plan

- [ ] Step 1: 冻结 Phase 1/2 的项目运行时模型。
  - 梳理现有 `project` 主表、`project_material_line`、`allocation_target = RD_PROJECT` 与 `PROJECT_CONSUMPTION_OUT` 现状。
  - 明确 `项目主档/BOM` 与 `项目库存动作` 的职责切分，禁止继续把主档 `CRUD` 当成直接领料操作。
...
- [ ] Step 4: 落地 `F3` 项目库存动作边界。
  - 将项目领料/退料/报废与 `inventory-core`、`RD_PROJECT` 归集、来源追溯收口到明确实现路径。
...
- Execution brief:
  - 先把 `project` 从“项目消耗单”纠偏到“项目主档 + BOM + 台账域”；不要一开始就在旧页面上堆叠更多字段而保留错误运行时语义。
...
- Owned paths:
  ...
  - 与本 task 直接相关的 `test/**`
  - `docs/acceptance-tests/specs/project-management.md`
  - `docs/acceptance-tests/cases/project-management.json`
  - `docs/acceptance-tests/runs/run-YYYYMMDD-HHMM-project-management-phase1-phase2.md`
...
- Constraints and non-goals:
  - 不实现 `F5` customer 销售成本快照。
  - 不引入项目状态机、排期、进度或完整项目管理能力。
  - 不允许 `BOM` 写路径直接过账库存。
  - 不允许绕过 `inventory-core` 直改库存。
  - 不允许在 `project` 与 `workshop-material` 之间形成两套长期并行且不一致的领料/退料/报废运行时合同。
  - 不改变 `RD_SUB` / `MAIN` 的职责边界，不把采购/验收主流程迁入 `project`。
- Validation command for this scope:
  - `pnpm prisma:validate`
  - `pnpm typecheck`
  - `pnpm test -- src/modules/project/application/project.service.spec.ts src/modules/inventory-core/application/inventory.service.spec.ts src/modules/rd-subwarehouse/application/rd-procurement-request.service.spec.ts src/modules/rd-subwarehouse/application/rd-handoff.service.spec.ts src/modules/workshop-material/application/workshop-material.service.spec.ts`
  - `pnpm test:e2e -- <focused project-management suites if added>`
...
- Review focus:
  - `project` 是否真的从“消耗单”提升为 `F1/F2/F4` 要求的项目域，而不是把旧消费语义换个名字继续保留。
...
- Requirement alignment check:
  ...
  - 若执行结果把 Phase 2 完全重心转移到 `workshop-material`，需确认 `project` 仍保有 requirement 要求的项目语义真源与查询能力。
- Final validation gate:
  ...
  - `pnpm test:e2e -- <focused project-management suites if added>`
  - `pnpm --dir web build:prod`
  - browser acceptance 证据 + `docs/acceptance-tests/specs/project-management.md` + `docs/acceptance-tests/cases/project-management.json` + `docs/acceptance-tests/runs/run-YYYYMMDD-HHMM-project-management-phase1-phase2.md`
...
- Evidence pointers:
  - `project` / `inventory-core` / `rd-subwarehouse` / `workshop-material` focused tests
...
- Validation results:
  ...
  - `pnpm typecheck` → `pass`
  - `pnpm test -- src/modules/project/application/project.service.spec.ts src/modules/project/application/project-material-action.service.spec.ts src/modules/rd-subwarehouse/application/rd-procurement-request.service.spec.ts src/modules/workshop-material/application/workshop-material.service.spec.ts` → `pass`
  - `pnpm --dir web build:prod` → `pass`
- Findings:
  - `no blocking code-review findings recorded in parent validation lane`
  - `runtime rollout prerequisite remains: sync Prisma schema before opening /api/projects in a live environment`
- Follow-up action:
...
- [x] `[AC-1]` 项目主档轻量 `CRUD` 与固定 `RD_SUB` 口径落地 — Evidence: `project.service.spec.ts` create 不再直接出库 + browser 创建项目主档成功 — Verdict: `✓ met`
- [x] `[AC-2]` 项目 `BOM`、缺料预警与采购辅助落地 — Evidence: browser 详情页展示缺口 / 补货状态 + `project.service.spec.ts` shortage / replenishment 聚合 — Verdict: `✓ met`
- [x] `[AC-3]` 项目库存动作通过 `inventory-core` 落账且与 `workshop-material` 边界一致 — Evidence: `project-material-action.service.spec.ts` 覆盖 `PICK/RETURN/SCRAP` 与回补/逆转，详情页展示独立动作入口 — Verdict: `✓ met`
- [x] `[AC-4]` 项目净耗用与成本台账可查询并可追溯 — Evidence: browser 详情页成本摘要与台账 + `project.service.spec.ts` ledger/cost 聚合 — Verdict: `✓ met`
- [x] `[AC-5]` 前端项目管理闭环可用 — Evidence: 菜单 / 工作台 / 页面标题统一为“项目管理”，并完成浏览器创建 + 详情 walkthrough — Verdict: `✓ met`
...
- Outcome:
  - `accepted`
- Requirement alignment:
  - `aligned to docs/requirements/domain/project-management.md (F1,F2,F3,F4)`
- Residual risks or testing gaps:
  - `本地 live 环境需先执行 Prisma schema sync；否则 /api/projects 会因缺少 ProjectBomLine / ProjectMaterialAction* 表而返回 500`
