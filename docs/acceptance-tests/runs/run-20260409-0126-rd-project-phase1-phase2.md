# 历史 RD 项目 Phase 1/2 验收报告

## 元数据

| 字段 | 值 |
|------|------|
| 关联spec | `docs/acceptance-tests/specs/rd-project.md` |
| 关联task | `docs/tasks/archive/retained-completed/task-20260409-0056-rd-project-phase1-phase2-delivery.md` |
...
> 历史说明：本报告冻结的是语义拆分前的 `RD` 项目实现证据。当前运行时文案已统一为“研发项目”，研发项目真源见 `docs/requirements/domain/rd-project-management.md`，销售项目真源见 `docs/requirements/domain/sales-project-management.md`。

## 执行记录

- 先执行 `set -a && source .env.dev && set +a && pnpm prisma:validate`，结果 `pass`。
- 首次访问 `/rd/projects` 时，列表请求 `/api/rd-projects?limit=10&offset=0` 返回 `500`；确认根因是本地 MySQL 尚未同步新增的 `RdProjectBomLine / RdProjectMaterialAction*` schema。
- 显式执行 `set -a && source .env.dev && set +a && pnpm prisma db push --schema prisma/schema.prisma --accept-data-loss` 后，页面恢复正常。
- 登录 `admin / admin123`，访问 `/rd/projects`：
  - 当时菜单项、面包屑、页标题均显示“项目管理”；当前运行时已改为“研发项目”。
...
| AC | 结论 | 关键证据 | 备注 |
|----|------|----------|------|
| AC-1 | `met` | 浏览器创建项目主档成功；`rd-project.service.spec.ts` 断言 create 不再直接出库 | 项目语义已从“项目领用单”升级为项目主档 |
| AC-2 | `met` | 详情页展示 BOM / 缺口 / 补货状态；service spec 覆盖 shortage / replenishment 聚合 | browser 样本走到“待补货”路径 |
| AC-3 | `met` | `rd-project-material-action.service.spec.ts` 覆盖 `PICK/RETURN/SCRAP` 与回补/逆转；浏览器详情页展示动作入口 | browser run 未额外准备 RD_SUB 库存 fixture |
| AC-4 | `met` | 详情页展示成本摘要与净耗用台账；service spec 覆盖成本聚合 | browser 当前样本为零金额基线 |
| AC-5 | `met` | 菜单、页面、创建弹窗、详情抽屉、动作页签均可真实访问 | 历史验收时工作台与菜单文案为“项目管理”，当前运行时已收口为“研发项目” |
| AC-6 | `met` | Prisma/typecheck/test/build/browser/spec/cases/run 证据齐全 | 需记录 schema sync 为环境前置条件 |

## 总结

- 建议：`accept`
- 残余风险：部署或其他本地库如果未先同步 Prisma schema，`/api/rd-projects` 仍会因缺表失败；当前 browser run 未准备非零成本的 live 库存动作样本，但该部分已有 focused tests 覆盖。
