import { buildSalesReturnMigrationPlan } from "../../scripts/migration/sales-return/transformer";
import type {
  LegacySalesReturnSnapshot,
  SalesReturnDependencySnapshot,
} from "../../scripts/migration/sales-return/types";
...
describe("sales-return migration transformer (formal-row-first)", () => {
  ...
  it("should admit a sales-return order that resolves to exactly one upstream outbound line with sourceDoc* populated", () => {
    ...
    expect(line?.target.sourceDocumentType).toBe("SalesStockOrder");
