...
async function upsertWorkshopMaterialOrder(
  connection: MigrationConnectionLike,
  record: WorkshopReturnOrderPlanRecord,
): Promise<number> {
  return runUpsert(
    connection,
    `
      ...
      ON DUPLICATE KEY UPDATE
        ...
        handler_personnel_id = VALUES(handler_personnel_id),
        ...
        handler_name_snapshot = VALUES(handler_name_snapshot),
...
async function upsertWorkshopMaterialOrderLine(
  connection: MigrationConnectionLike,
  orderId: number,
  record: WorkshopReturnLinePlanRecord,
): Promise<number> {
  return runUpsert(
    connection,
    `
...
async function insertExcludedDocument(
  connection: MigrationConnectionLike,
  migrationBatch: string,
  record: ExcludedWorkshopReturnPlanRecord,
): Promise<void> {
  await connection.query(
    `
...
export async function executeWorkshopReturnPlan(
  ...
  try {
    ...
    for (const record of plan.pendingRelations) {
      await insertPendingRelation(connection, plan.migrationBatch, record);
      pendingRelationCount += 1;
    }
