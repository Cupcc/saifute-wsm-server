...
import type {
  ...
  StockInGlobalBlocker,
  StockInLinePlanRecord,
  StockInMigrationPlan,
  StockInOrderPlanRecord,
  StockInOrderSourceTable,
  StockInOrderTargetInsert,
  StockInOrderTypeValue,
  StockInPlanCounts,
  StockInWarning,
} from "./types";
...
interface PreparedDependencies {
  supplier: ResolvedSupplierDependency | null;
...
function buildOrderKey(
  order: Pick<LegacyStockInOrderRow, "legacyTable" | "legacyId">,
): string {
...
function sortByActiveFirstThenLegacyId(
  left: LegacyStockInOrderRow,
  right: LegacyStockInOrderRow,
): number {
...
function resolveDeterministicDocumentNumbers(
  orders: readonly LegacyStockInOrderRow[],
): {
  ...
  const groupedOrders = new Map<
    string,
    Array<LegacyStockInOrderRow & { normalizedDocumentNo: string }>
  >();
  ...
  return {
    ...
    rewrites: rewrites.sort(
      (left, right) =>
        ...
        left.keptLegacyTable.localeCompare(right.keptLegacyTable),
    ),
  };
}
...
function normalizeBizDate(value: string | null | undefined): string | null {
  const normalized = normalizeOptionalText(value);
...
function resolveOrderType(
  ...
  return legacyTable === "saifute_inbound_order"
    ? "ACCEPTANCE"
    : "PRODUCTION_RECEIPT";
}
...
function buildLinesByOrderKey(
  lines: readonly LegacyStockInLineRow[],
): Map<string, LegacyStockInLineRow[]> {
  const linesByOrderKey = new Map<string, LegacyStockInLineRow[]>();
...
function buildOrderArchivedPayload(
  ...
  return {
    ...
    archiveReason:
      "Archive source-only stock-in order fields and original document number.",
    payload: {
      originalDocumentNo: normalizeOptionalText(order.sourceDocumentNo),
      sourceOrderTypeFieldName:
        order.legacyTable === "saifute_inbound_order"
          ? "inbound_type"
          : "into_type",
      sourceOrderTypeFieldValue: order.sourceOrderTypeValue ?? null,
...
function buildLineArchivedPayload(
  line: LegacyStockInLineRow,
  targetCode: string,
): ArchivedFieldPayloadRecord | null {
  const payload = {
    taxPrice:
      line.legacyTable === "saifute_inbound_detail"
        ? normalizeDecimalToScale(line.taxPrice, 2)
        : null,
    interval:
      line.legacyTable === "saifute_into_detail"
        ? normalizeOptionalText(line.interval)
        : null,
  };

  if (!payload.taxPrice && !payload.interval) {
    return null;
  }
  ...
  return {
    ...
    targetCode,
    payloadKind: "legacy-unmapped-fields",
    archiveReason: "Archive source-only stock-in line fields.",
    payload,
  };
}
...
function buildExcludedDocumentPlan(
  ...
  return {
    ...
    payload: {
      ...
      orderType: resolveOrderType(order.legacyTable),
      sourceOrderTypeFieldValue: order.sourceOrderTypeValue ?? null,
      bizDate: normalizeBizDate(order.bizDate),
      totalAmount: normalizeDecimalToScale(order.totalAmount, 2),
      supplierLegacyId: normalizePositiveLegacyId(order.supplierLegacyId),
      workshopLegacyId: normalizePositiveLegacyId(order.workshopLegacyId),
      handlerName: normalizeOptionalText(order.handlerName),
      chargeBy: normalizeOptionalText(order.chargeBy),
      ...
      lines: lines.map((line) => ({
        ...
        taxPrice: normalizeDecimalToScale(line.taxPrice, 2),
...
function pushGlobalBlockers(
  ...
  dependencies: StockInDependencySnapshot,
): void {
...
function pushDependencyWarnings(
  ...
  for (const ambiguousName of Array.from(
    ...
    warnings.push({
      ...
      legacyId: null,
      reason:
        "Target personnel snapshot contains ambiguous names; matching documents will be excluded.",
      details: {
...
function resolveDependencies(
  ...
  const supplierLegacyId = normalizePositiveLegacyId(order.supplierLegacyId);
  const workshopLegacyId = normalizePositiveLegacyId(order.workshopLegacyId);
  const normalizedHandlerName = normalizePersonnelLookupName(order.handlerName);

  let supplier: ResolvedSupplierDependency | null = null;
  if (supplierLegacyId !== null) {
    supplier =
      dependencies.supplierByLegacyKey.get(
        buildLegacyKey("saifute_supplier", supplierLegacyId),
      ) ?? null;

    if (!supplier) {
      exclusionReasons.push(
        `Supplier dependency ${supplierLegacyId} is missing from the batch1 supplier map.`,
      );
    }
  }
  ...
  if (workshopLegacyId !== null) {
    ...
    if (!workshop) {
      exclusionReasons.push(
        `Workshop dependency ${workshopLegacyId} is missing from the batch1 workshop map.`,
      );
    }
  } else {
    ...
    if (!workshop) {
      exclusionReasons.push(
        "Document is missing workshop_id and the frozen default workshop is unavailable.",
      );
    }
  }
  ...
  if (normalizedHandlerName !== null) {
    if (dependencies.ambiguousPersonnelNames.has(normalizedHandlerName)) {
      warnings.push({
        legacyTable: order.legacyTable,
        legacyId: order.legacyId,
        reason:
      ...
      handler =
        dependencies.personnelByNormalizedName.get(normalizedHandlerName) ??
        null;
  ...
  return {
    ...
    workshop,
    handler,
    handlerNameSnapshot: handler?.personnelName ?? normalizedHandlerName,
  };
}
...
function buildCounts(
  snapshot: LegacyStockInSnapshot,
  migratedOrders: readonly StockInOrderPlanRecord[],
  excludedDocuments: readonly ExcludedDocumentPlanRecord[],
): StockInPlanCounts {
  return {
    orders: {
    ...
    sourceOrderTables: {
      saifute_inbound_order: snapshot.orders.filter(
        (order) => order.legacyTable === "saifute_inbound_order",
      ).length,
      saifute_into_order: snapshot.orders.filter(
        (order) => order.legacyTable === "saifute_into_order",
      ).length,
    },
    sourceLineTables: {
      saifute_inbound_detail: snapshot.lines.filter(
        (line) => line.legacyTable === "saifute_inbound_detail",
      ).length,
      saifute_into_detail: snapshot.lines.filter(
        (line) => line.legacyTable === "saifute_into_detail",
      ).length,
    },
  };
}

export function buildStockInMigrationPlan(
  snapshot: LegacyStockInSnapshot,
  dependencies: StockInDependencySnapshot,
): StockInMigrationPlan {
  const warnings: StockInWarning[] = [];
  const globalBlockers: StockInGlobalBlocker[] = [];
  const migratedOrders: StockInOrderPlanRecord[] = [];
  const excludedDocuments: ExcludedDocumentPlanRecord[] = [];
  ...
  for (const order of [...snapshot.orders].sort(
    ...
    const bizDate = normalizeBizDate(order.bizDate);
    if (!bizDate) {
    ...
    const orderTarget: StockInOrderTargetInsert = {
      documentNo: targetDocumentNoCandidate,
      orderType: resolveOrderType(order.legacyTable),
      bizDate,
      supplierId: resolvedDependencies.supplier?.targetId ?? null,
      ...
      revisionNo: 1,
      supplierCodeSnapshot: resolvedDependencies.supplier?.supplierCode ?? null,
      supplierNameSnapshot: resolvedDependencies.supplier?.supplierName ?? null,
    ...
    const linePlans: StockInLinePlanRecord[] = preparedLines.map(
      (preparedLine, index) => {
        const lineNo = index + 1;
        const lineTargetCode = `${targetDocumentNoCandidate}#${lineNo}`;
  ...
  return {
    migrationBatch: STOCK_IN_MIGRATION_BATCH,
...
export function hasExecutionBlockers(plan: StockInMigrationPlan): boolean {
