...
export function buildSliceDirtyTargetBlockers(
  ...
  if (state.batchOwnedTargetRows === 0 && state.batchMapRows === 0) return [];
  if (state.batchOwnedTargetRows === state.batchMapRows) return [];

  return [
    {
      reason:
        "Batch-owned workshop-scrap target rows and this batch's staging map rows differ, so execute would run against a dirty slice baseline.",
...
export function buildMissingMapTargetBlockers(
  ...
  if (state.missingMappedTargets === 0) return [];

  return [
    {
      reason:
        "This batch's workshop-scrap staging map rows point at missing target rows, so execute would run against an inconsistent slice baseline.",
      targetTable: state.targetTable,
...
export function buildMapConsistencyBlockers(
  ...
  return [
    {
      reason:
        "This batch's workshop-scrap staging map rows do not match the deterministic plan, so rerun cleanup would be unsafe.",
      targetTable: state.targetTable,
  ...
  return [
    {
      reason: state.hasBatchOwnership
        ? "Workshop-scrap rerun is blocked because downstream tables already reference WorkshopMaterialOrder rows or line ids."
        : "Workshop-scrap execute is blocked because downstream WorkshopMaterialOrder consumers already exist before this slice owns the target rows.",
