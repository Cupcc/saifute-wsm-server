...
import {
  buildDryRunSummary,
  buildScrapMigrationPlan,
  hasExecutionBlockers,
} from "./transformer";
import { executeScrapPlan, MAP_TABLES, TARGET_TABLES } from "./writer";
...
async function getMissingMapTargets(
  ...
  const rows = await connection.query<Array<{ total: number }>>(
    `
      ...
      LEFT JOIN ${targetTableName} target_row ON target_row.id = map_row.target_id
      WHERE map_row.migration_batch = ? AND target_row.id IS NULL
    `,
    [migrationBatch],
  );
  return Number(rows[0]?.total ?? 0);
...
async function main(): Promise<void> {
  ...
  const reportPath = resolveReportPath(
    cliOptions,
    cliOptions.execute
      ? "scrap-execute-report.json"
      : "scrap-dry-run-report.json",
  );
  ...
  try {
    ...
    if (!cliOptions.execute) {
      ...
      console.log(`Scrap dry-run completed. report=${reportPath}`);
    ...
    const executionReport = await withPoolConnection(
      ...
      async (targetConnection) => {
        ...
        const executionResult = await executeScrapPlan(targetConnection, plan);
        const report = {
          ...dryRunReport,
          executionRequested: true,
          stagingReady,
    ...
    console.log(`Scrap execute completed. report=${reportPath}`);
