  ...
  >(
    `
      SELECT
        ...
        line_row.selected_unit_cost AS selectedUnitCost,
        ...
        for (const [tableName, total] of Object.entries(forbiddenTableCounts)) {
          if (total > 0) {
            validationIssues.push({
              severity: "blocker",
              reason:
                "A table that must remain untouched by the outbound base slice already contains SalesStockOrder-linked rows.",
              tableName,
              total,
            });
          }
        ...
        for (const order of plan.migratedOrders) {
          ...
          for (const line of order.lines) {
            ...
            pushValueMismatch(
              validationIssues,
              lineContext,
              "sales_stock_order_line.selectedUnitCost",
              line.target.selectedUnitCost,
              targetLine.selectedUnitCost,
            );
            pushValueMismatch(
              validationIssues,
              lineContext,
