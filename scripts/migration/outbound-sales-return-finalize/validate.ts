...
} from "../outbound-sales-return/legacy-reader";
import { buildSalesReturnMigrationPlan } from "../outbound-sales-return/transformer";
...
async function getForbiddenTableCounts(connection: {
  ...
  >(
    `
      ...
      INNER JOIN customer_stock_order cso
        ON cso.id = fnr.businessDocumentId
      WHERE fnr.businessDocumentType = 'CustomerStockOrder'
...
async function main(): Promise<void> {
  ...
  const reportPath = resolveReportPath(
    cliOptions,
    "outbound-sales-return-finalize-validate-report.json",
  );
      ...
      async (targetConnection) => {
        ...
        if (
          ...
          validationIssues.push({
            severity: "blocker",
            reason:
              "map_customer_stock_order row count changed during finalization.",
        ...
        if (stagingReady && batchLineMapRows !== expectedBatchOwnedLineCount) {
          validationIssues.push({
            severity: "blocker",
            reason:
              "map_customer_stock_order_line row count changed during finalization.",
        ...
        if (
          ...
          validationIssues.push({
            severity: "blocker",
            reason:
              "Batch-owned customer_stock_order row count drifted from the deterministic batch3c baseline.",
            expectedBatchOwnedOrderCount,
        ...
        if (
          ...
          validationIssues.push({
            severity: "blocker",
            reason:
              "Batch-owned customer_stock_order_line row count drifted from the deterministic batch3c baseline.",
            expectedBatchOwnedLineCount,
        ...
        for (const [tableName, total] of Object.entries(forbiddenTableCounts)) {
          if (total > 0) {
            validationIssues.push({
              severity: "blocker",
              reason:
                "A table that must remain untouched by the finalization slice already contains CustomerStockOrder-linked rows.",
              tableName,
              total,
            });
          }
