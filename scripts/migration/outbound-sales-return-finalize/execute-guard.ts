...
export function buildDisallowedReasonBlockers(
  ...
  return [
    {
      reason:
        "batch3c pending_relations contains rows with a disallowed reason family other than 'no-upstream-line-candidate'. This finalization slice only handles that specific reason. A new planning step is required before these rows can be finalized.",
...
export function buildPendingDriftBlockers(
  ...
  if (state.currentPendingRows.length !== state.expectedRelations.length) {
    blockers.push({
      reason:
        "Current batch3c pending_relations count differs from the deterministic plan. The finalization plan cannot be safely built.",
  ...
  if (missingExpected.length > 0) {
    blockers.push({
      reason:
        "Expected batch3c pending_relations rows are missing from the DB. Payload drift detected.",
  ...
  if (unexpectedRows.length > 0) {
    blockers.push({
      reason:
        "batch3c pending_relations contains rows not in the deterministic plan. Unexpected staging rows detected.",
  ...
  if (reasonMismatches.length > 0) {
    blockers.push({
      reason:
        "batch3c pending_relations rows have mismatched pending_reason compared to the deterministic plan.",
  ...
  if (payloadMismatches.length > 0) {
    blockers.push({
      reason:
        "batch3c pending_relations payload_json does not match the deterministic plan. Payload drift detected.",
