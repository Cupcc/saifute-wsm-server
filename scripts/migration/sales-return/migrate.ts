...
async function main(): Promise<void> {
  ...
  const reportPath = resolveReportPath(
    cliOptions,
    cliOptions.execute
      ? "outbound-sales-return-execute-report.json"
      : "outbound-sales-return-dry-run-report.json",
  );
  ...
  try {
    ...
    if (!cliOptions.execute) {
      ...
      console.log(`Sales-return dry-run completed. report=${reportPath}`);
      ...
      async (targetConnection) => {
        ...
        if (!stagingReady) {
          ...
          executeBlockers.push(
            ...
            ...buildMapConsistencyBlockers({
              targetTable: TARGET_TABLES.order,
              allowMissingExpectedMapRows: isRerun,
              allowUnexpectedMapRows: isRerun,
              ...orderMapConsistency,
            }),
            ...buildMapConsistencyBlockers({
              targetTable: TARGET_TABLES.line,
        ...
        const executionResult = await executeSalesReturnPlan(
          targetConnection,
          plan,
        );
        const report = {
    ...
    console.log(`Sales-return execute completed. report=${reportPath}`);
