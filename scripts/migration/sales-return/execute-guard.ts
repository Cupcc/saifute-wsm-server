...
export function buildDownstreamConsumerBlockers(
  ...
  const reason = state.isRerun
    ? "Sales-return rerun is blocked because downstream tables already reference SalesStockOrder rows or line ids created or modified by previous slices."
    : "Sales-return first execute is blocked because downstream tables that depend on the SalesStockOrder baseline are already populated; executing would risk data corruption when the baseline is rebuilt.";
