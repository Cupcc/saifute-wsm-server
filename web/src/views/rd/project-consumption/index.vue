<template>
  <div class="app-container project-page">
    <el-card shadow="never" class="hero-card">
      <div class="hero">
        <div>
          <div class="hero-eyebrow">RD_SUB 项目管理</div>
          <h1 class="hero-title">项目主档、BOM、缺料与台账</h1>
          <p class="hero-copy">
            统一管理研发项目主档，维护 BOM，查看缺料补货状态，并记录项目领料、退料、报废事实。
          </p>
        </div>
    ...
    <el-card shadow="never" class="panel-card">
      ...
      <div class="toolbar">
        <div class="toolbar-copy">
          <div class="toolbar-title">项目主档</div>
          <div class="toolbar-subtitle">BOM 保存不会直接过账库存，库存动作在项目台账中单独记录。</div>
        </div>
    ...
    >
      <el-form label-width="100px" class="project-form">
        ...
        <div class="section-toolbar">
          <div>
            <div class="section-title">项目 BOM</div>
      ...
      <template v-if="detailRow">
        <div class="detail-shell">
          ...
          <el-tabs v-model="detailTab" class="detail-tabs">
            ...
            <el-tab-pane label="项目物料动作" name="actions">
              <div class="section-toolbar compact">
                <div>
                  <div class="section-title">领料 / 退料 / 报废</div>
                  <div class="section-subtitle">库存事实统一通过 inventory-core 落账，并持续挂到当前项目。</div>
                </div>
                <el-button type="primary" @click="openActionDialog">新增物料动作</el-button>
              </div>
          ...
          <el-col :span="12">
            <el-form-item label="动作类型">
              <el-select v-model="actionForm.actionType" style="width: 100%">
                <el-option label="项目领料" value="PICK" />
                <el-option label="项目退料" value="RETURN" />
                <el-option label="项目报废" value="SCRAP" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
<script setup name="RdProjectManagementPage">
...
async function handleVoidProject(projectId) {
  try {
    const result = await ElMessageBox.prompt("请输入作废原因", "作废项目", {
  ...
  try {
    await createRdProjectMaterialAction(selectedProjectId.value, {
      ...
      lines: actionForm.value.lines.map((line) => ({
        ...
        unitPrice: String(line.unitPrice || 0),
        sourceDocumentType:
          actionForm.value.actionType === "RETURN" ? "ProjectMaterialAction" : undefined,
        sourceDocumentId:
...
async function handleVoidAction(actionId) {
  try {
    const result = await ElMessageBox.prompt("请输入作废原因", "作废项目物料动作", {
      confirmButtonText: "确认",
      cancelButtonText: "取消",
      inputValue: "项目动作作废",
    });
