...
export class InMemoryRbacRepository {
  ...
  ensureSeedPermissionMenus(
    ...
    return this.seedRepairRepo.ensureSeedPermissionMenus(
      roleKeys,
      permissionKeys,
    );
  }
  syncSeedRoleMenus(roleKeys: string[]): boolean {
    return this.seedRepairRepo.syncSeedRoleMenus(roleKeys);
  }
  ...
  async updateUserAvatar(userId: number, avatarUrl: string | null) {
    const result = await this.userRepo.updateUserAvatar(userId, avatarUrl);
  ...
  getUserForm(userId: number | null) {
    return this.userRepo.getUserForm(userId);
  }
  createUser(data: Record<string, unknown>) {
    const result = this.userRepo.createUser(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  updateUser(data: Record<string, unknown>) {
    const result = this.userRepo.updateUser(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  deleteUsers(userIds: number[]) {
    this.userRepo.deleteUsers(userIds);
    this.persistenceRepo.queuePersistence();
  }
  ...
  getCurrentUserProfile(userId: number) {
    return this.userRepo.getCurrentUserProfile(userId);
  }
  updateCurrentUserProfile(
    userId: number,
    data: Pick<ManagedUserRecord, "nickName" | "phonenumber" | "email" | "sex">,
  ) {
    const result = this.userRepo.updateCurrentUserProfile(userId, data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  ...
  getAuthRole(userId: number) {
    return this.userRepo.getAuthRole(userId);
  }
  ...
  findUserIdsByRoleIds(roleIds: number[]) {
    return this.userRepo.findUserIdsByRoleIds(roleIds);
  }
  listAllocatedUsers(query: Record<string, string | undefined>) {
    return this.userRepo.listAllocatedUsers(query);
  }
  listUnallocatedUsers(query: Record<string, string | undefined>) {
    return this.userRepo.listUnallocatedUsers(query);
  }
  ...
  listRoles(query: Record<string, string | undefined>) {
    return this.resourceRepo.listRoles(query);
  }
  getRole(roleId: number) {
    return this.resourceRepo.getRole(roleId);
  }
  createRole(data: Record<string, unknown>) {
    const result = this.resourceRepo.createRole(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  updateRole(data: Record<string, unknown>) {
    const result = this.resourceRepo.updateRole(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  updateRoleDataScope(data: Record<string, unknown>) {
    this.resourceRepo.updateRoleDataScope(data);
    this.persistenceRepo.queuePersistence();
  }
  ...
  deleteRoles(roleIds: number[]) {
    this.resourceRepo.deleteRoles(roleIds);
    this.persistenceRepo.queuePersistence();
  }
  getRoleMenuTree(roleId: number) {
    return this.resourceRepo.getRoleMenuTree(roleId);
  }
  listMenus(query: Record<string, string | undefined>) {
    return this.resourceRepo.listMenus(query);
  }
  getMenu(menuId: number) {
    return this.resourceRepo.getMenu(menuId);
  }
  getMenuTreeSelect() {
    return this.resourceRepo.getMenuTreeSelect();
  }
  createMenu(data: Record<string, unknown>) {
    const result = this.resourceRepo.createMenu(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  updateMenu(data: Record<string, unknown>) {
    const result = this.resourceRepo.updateMenu(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  deleteMenus(menuIds: number[]) {
    this.resourceRepo.deleteMenus(menuIds);
    this.persistenceRepo.queuePersistence();
  }
  listDepts(query: Record<string, string | undefined>) {
    return this.resourceRepo.listDepts(query);
  }
  listDeptExcludeChild(deptId: number) {
    return this.resourceRepo.listDeptExcludeChild(deptId);
  }
  getDept(deptId: number) {
    return this.resourceRepo.getDept(deptId);
  }
  createDept(data: Record<string, unknown>) {
    const result = this.resourceRepo.createDept(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  updateDept(data: Record<string, unknown>) {
    const result = this.resourceRepo.updateDept(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  deleteDepts(deptIds: number[]) {
    this.resourceRepo.deleteDepts(deptIds);
    this.persistenceRepo.queuePersistence();
  }
  getDeptTree(roleId?: number) {
    return this.resourceRepo.getDeptTree(roleId);
  }
  getDeptTreeSelect() {
    return this.resourceRepo.getDeptTreeSelect();
  }
  listPosts(query: Record<string, string | undefined>) {
    return this.resourceRepo.listPosts(query);
  }
  getPost(postId: number) {
    return this.resourceRepo.getPost(postId);
  }
  createPost(data: Record<string, unknown>) {
    const result = this.resourceRepo.createPost(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  updatePost(data: Record<string, unknown>) {
    const result = this.resourceRepo.updatePost(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  deletePosts(postIds: number[]) {
    this.resourceRepo.deletePosts(postIds);
    this.persistenceRepo.queuePersistence();
  }

  listDictTypes(query: Record<string, string | undefined>) {
    return this.dictConfigRepo.listDictTypes(query);
  }
  getDictType(dictId: number) {
    return this.dictConfigRepo.getDictType(dictId);
  }
  createDictType(data: Record<string, unknown>) {
    const result = this.dictConfigRepo.createDictType(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  updateDictType(data: Record<string, unknown>) {
    const result = this.dictConfigRepo.updateDictType(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  deleteDictTypes(dictIds: number[]) {
    this.dictConfigRepo.deleteDictTypes(dictIds);
    this.persistenceRepo.queuePersistence();
  }
  listDictTypeOptions() {
    return this.dictConfigRepo.listDictTypeOptions();
  }
  listDictData(query: Record<string, string | undefined>) {
    return this.dictConfigRepo.listDictData(query);
  }
  getDictData(dictCode: number) {
    return this.dictConfigRepo.getDictData(dictCode);
  }
  getDictDataByType(dictType: string) {
    return this.dictConfigRepo.getDictDataByType(dictType);
  }
  createDictData(data: Record<string, unknown>) {
    const result = this.dictConfigRepo.createDictData(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  updateDictData(data: Record<string, unknown>) {
    const result = this.dictConfigRepo.updateDictData(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  ...
  listConfigs(query: Record<string, string | undefined>) {
    return this.dictConfigRepo.listConfigs(query);
  }
  getConfig(configId: number) {
    return this.dictConfigRepo.getConfig(configId);
  }
  getConfigByKey(configKey: string) {
    return this.dictConfigRepo.getConfigByKey(configKey);
  }
  createConfig(data: Record<string, unknown>) {
    const result = this.dictConfigRepo.createConfig(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  updateConfig(data: Record<string, unknown>) {
    const result = this.dictConfigRepo.updateConfig(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  deleteConfigs(configIds: number[]) {
    this.dictConfigRepo.deleteConfigs(configIds);
    this.persistenceRepo.queuePersistence();
  }
  listNotices(query: Record<string, string | undefined>) {
    return this.dictConfigRepo.listNotices(query);
  }
  getNotice(noticeId: number) {
    return this.dictConfigRepo.getNotice(noticeId);
  }
  createNotice(data: Record<string, unknown>) {
    const result = this.dictConfigRepo.createNotice(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  updateNotice(data: Record<string, unknown>) {
    const result = this.dictConfigRepo.updateNotice(data);
    this.persistenceRepo.queuePersistence();
    return result;
  }
  deleteNotices(noticeIds: number[]) {
    this.dictConfigRepo.deleteNotices(noticeIds);
    this.persistenceRepo.queuePersistence();
  }
