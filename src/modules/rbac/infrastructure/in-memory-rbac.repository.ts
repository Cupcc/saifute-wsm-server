...
export class InMemoryRbacRepository {
  ...
  ensureSeedPermissionMenus(
    ...
    const existingPermissions = new Set(
      this.menus.map((menu) => menu.perms).filter(Boolean),
    );
    ...
    for (const seedMenu of requiredSeedMenus) {
      const currentMenu = this.menus.find((menu) => menu.perms === seedMenu.perms);
      if (currentMenu) {
      ...
      const conflictingMenu = this.menus.find(
        (menu) =>
    ...
    for (const roleKey of roleKeys) {
      const role = this.roles.find((item) => item.roleKey === roleKey);
      if (!role) {
        continue;
      }
  ...
  syncSeedRoleMenus(roleKeys: string[]): boolean {
    const seedState = createSystemManagementSeedState();
    const currentMenuIds = new Set(this.menus.map((menu) => menu.menuId));
    let changed = false;
