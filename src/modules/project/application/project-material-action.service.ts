...
import type { CreateProjectMaterialActionLineDto } from "../dto/create-project-material-action-line.dto";
import { ProjectRepository } from "../infrastructure/project.repository";
import {
  createProjectActionDocumentNo,
  ensureProjectAllocationTarget,
  FIXED_PROJECT_STOCK_SCOPE,
  PROJECT_ACTION_DOCUMENT_TYPE,
...
@Injectable()
export class ProjectMaterialActionService {
  constructor(
    ...
    const enriched = await Promise.all(
      actions.map(async (action) => {
        const returnedQtyMap =
          action.actionType === ProjectMaterialActionType.PICK
  ...
  async getMaterialActionById(actionId: number) {
    ...
    const lines = await Promise.all(
      action.lines.map(async (line) => {
        const sourceUsages =
          action.actionType === ProjectMaterialActionType.PICK ||
          action.actionType === ProjectMaterialActionType.SCRAP
    ...
    return createProjectActionDocumentNo(
      ...
      async (documentNo) =>
        this.prisma.runInTransaction(async (tx) => {
          const allocationTargetId = await ensureProjectAllocationTarget({
            project,
            updatedBy: createdBy,
          ...
          if (
            dto.actionType === ProjectMaterialActionType.PICK ||
            dto.actionType === ProjectMaterialActionType.SCRAP
          ) {
            const sourceTypes =
              FIXED_PROJECT_STOCK_SCOPE === "RD_SUB"
            ...
            for (const line of action.lines) {
              const settlement = await this.inventoryService.settleConsumerOut(
                {
                  materialId: line.materialId,
                  stockScope: FIXED_PROJECT_STOCK_SCOPE,
                  quantity: line.quantity,
                  operationType: toProjectInventoryOperationType(
                    dto.actionType,
                  ),
                  businessModule: PROJECT_BUSINESS_MODULE,
  ...
  async voidMaterialAction(
    ...
    return this.prisma.runInTransaction(async (tx) => {
      ...
      if (action.actionType === ProjectMaterialActionType.SCRAP) {
        await this.inventoryService.releaseAllSourceUsagesForConsumer(
          {
            consumerDocumentType: PROJECT_ACTION_DOCUMENT_TYPE,
            consumerDocumentId: action.id,
        ...
        await this.inventoryService.reverseStock(
          {
            ...
            idempotencyKey: `${PROJECT_ACTION_DOCUMENT_TYPE}:void:${action.id}:log:${log.id}`,
            note: `作废项目物料动作: ${action.documentNo}`,
          },
          tx,
        );
      }

      if (action.actionType === ProjectMaterialActionType.RETURN) {
        for (const line of action.lines) {
          if (
  ...
  private async prepareActionLines(
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      if (actionType === ProjectMaterialActionType.RETURN) {
        if (
          line.sourceDocumentType !== PROJECT_ACTION_DOCUMENT_TYPE ||
          !line.sourceDocumentId ||
          !line.sourceDocumentLineId
        ) {
          throw new BadRequestException("项目退料必须关联上游项目领料行");
        }
        ...
        if (sourceAction.actionType !== ProjectMaterialActionType.PICK) {
          throw new BadRequestException("退料来源必须是项目领料动作");
        }
        if (
