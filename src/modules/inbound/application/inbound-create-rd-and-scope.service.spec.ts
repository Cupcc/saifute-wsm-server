...
describe("InboundService", () => {
  ...
  it("should block void when source log has unreleased allocations", async () => {
    ...
    (repository.hasActiveDownstreamDependencies as jest.Mock).mockResolvedValue(
      false,
    );
    (inventoryService.getLogsForDocument as jest.Mock).mockResolvedValue([
      { id: 1 },
    ]);
    (inventoryService.hasUnreleasedAllocations as jest.Mock).mockResolvedValue(
      true,
    );

    await expect(service.voidOrder(1, "test reason", "admin")).rejects.toThrow(
      /已有下游消耗分配/,
    );
  });
